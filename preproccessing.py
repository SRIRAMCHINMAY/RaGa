...
randomly samples sequences of length(lth) from an input of sequences(seqs). For Data Augmentation.
Returns a list.
Args:
    seqs(list): list of sequences in the event vocabulary
    lth(int): approx length to cut sequences into
...
def aug(data, note_shifts=None,time_strectes=None, verbose=False):
    """
    ...
    if time_strectes is None:
        time_strectes=[1,1.05,1.1]
    if any([i<=0 for i in time_strectes]):
        raise ValueError("time_stretches must be positive")
    
    if 1 not in time_strectes:
        time_strectes.append(1)
    ts=[]
    for t in time_strectes:
        ts.append(t) if t not in ts else None
        ts.append(1/t) if (t != t and 1/t not in ts ) else None
    ts.sort()
    time_strectes=ts
    ...
    for seq in data:
        ...
        for seq in note_shifted_data:
            ...
            for time_stretch in time_strectes:
